...
func (s *Server) serveAuthenticatedSocket(ctx context.Context, connection protocolSocket, authenticate func(message) socketAuthentication, reliableStreams bool, channels <-chan protocolSocket) {
	...
	for {
		...
		case "process":
			processes.Handle(processRequest(request))
		case "rpc":
...
func filesystem(method string, args []any, body []byte) (any, error) {
	return fs.Call(method, args, body)
}

func (s *Server) handleExec(c protocolSocket, ctx context.Context, processes *proc.Set, request message) {
	...
	if request.ID == "" || strings.TrimSpace(request.File) == "" || len(request.File) > proc.MaxArgumentBytes || len(request.Command) > proc.MaxArgumentBytes || len(request.Cwd) > proc.MaxArgumentBytes || len(request.Args) > proc.MaxArgs {
	...
	for i, value := range request.Args {
		...
		if !ok || len(text) > proc.MaxArgumentBytes {
			send(c, ctx, map[string]any{"type": "exec-error", "id": request.ID, "error": "Executable request is invalid"})
			return
		}
		args[i] = text
	...
	if request.Stream {
		processes.Start(processRequest(request), args)
		return
	}
	execContext, cancel := context.WithTimeout(ctx, proc.MaxDuration)
	defer cancel()
	command := exec.CommandContext(execContext, request.File, args...)
	if request.Cwd != "" {
		command.Dir = request.Cwd
	}
	if err := proc.ApplyEnv(command, request.Env); err != nil {
		send(c, ctx, map[string]any{"type": "exec-error", "id": request.ID, "error": err.Error()})
		return
	}
	var stdout, stderr proc.BoundedBuffer
	stdout.Limit, stderr.Limit = proc.MaxOutputBytes, proc.MaxOutputBytes
