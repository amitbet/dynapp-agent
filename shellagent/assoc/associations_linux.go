...
func platformAssociationExtensions(appID string) ([]string, error) {
	...
	if runtime.GOOS == "darwin" {
		...
		for _, entry := range entries {
			...
			text := string(data)
			if strings.Contains(text, "com.dynapp.pwa."+agentutil.SafeName(appID)+"</string>") {
				section := text
	...
	data, err := os.ReadFile(filepath.Join(home, ".local", "share", "mime", "packages", "dynapp-"+agentutil.SafeName(appID)+".xml"))
...
func removeLinuxAssociations(appID string) error {
	...
	_ = os.Remove(filepath.Join(home, ".local", "share", "applications", "dynapp-"+agentutil.SafeName(appID)+".desktop"))
	_ = os.Remove(filepath.Join(home, ".local", "share", "mime", "packages", "dynapp-"+agentutil.SafeName(appID)+".xml"))
	_ = exec.Command("update-mime-database", filepath.Join(home, ".local", "share", "mime")).Run()
	return nil
}
