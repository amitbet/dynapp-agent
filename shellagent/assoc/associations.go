...
func (options AssociationOptions) Validate() error {
	if agentutil.SafeName(options.AppID) != options.AppID || options.AppID == "" {
	...
	for _, extension := range options.Extensions {
		if agentutil.SafeName(extension) != extension || extension == "" {
...
func AssociationState(appID string) (map[string]any, error) {
	...
	if runtime.GOOS == "darwin" && installed {
		applied, err = macDefaultAssociations("get", "com.dynapp.pwa."+agentutil.SafeName(appID), extensions)
		if err != nil {
			return nil, err
		}
	}
...
func UniqueExtensions(values []string) []string { return uniqueExtensions(values) }
...
func installDarwinAssociations(options AssociationOptions) error {
	...
	bundle := filepath.Join(home, "Applications", "DynApp", agentutil.SafeName(options.Name)+".app")
