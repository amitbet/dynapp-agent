...
func TestAssociationFallbackReportsInstalledExtensions(t *testing.T) {
	...
	if runtime.GOOS == "darwin" {
		previous := assoc.RunMacAssociationCommand
		t.Cleanup(func() { assoc.RunMacAssociationCommand = previous })
		assoc.RunMacAssociationCommand = func(args ...string) ([]byte, error) { return []byte(args[7]), nil }
	}
	if runtime.GOOS == "windows" {
