...
func remotePath(value any) (string, error) {
	parts := strings.FieldsFunc(strings.ReplaceAll(stringValue(value), "\\", "/"), func(r rune) bool { return r == '/' })
	clean := []string{}
...
func connectionValues(value any) (host, user, password, identity string, port int, err error) {
	...
	host, err = ftpControlArgument(strings.TrimSpace(stringValue(record["host"])), "host")
	if err != nil {
		return
	}
...
func (s *Server) handleNetworkFileRPC(ctx context.Context, socket protocolSocket, request message) (any, error) {
	...
	// Capability crossovers: touching local files through a network client
	// needs the matching local filesystem permission on the same socket.
	...
	client, err := s.openNetworkFileClient(ctx, request.Service, request.Args[0])
	if err != nil {
		return nil, err
	}
	defer client.Close()
	pathArg := func(index int) (string, error) {
		...
		return remotePath(request.Args[index])
	}
	...
	if len(request.Args) > 3 {
		progressID = stringValue(request.Args[len(request.Args)-1])
	}
	...
	case "readText", "readBase64":
		...
		if len(request.Args) > 2 {
			if value, ok := request.Args[2].(float64); ok && value > 0 {
				limit = int64(value)
			}
		}
	...
	case "readChunk":
		...
		if value, ok := request.Args[2].(float64); ok {
			offset = int64(value)
		}
		if value, ok := request.Args[3].(float64); ok {
			length = int64(value)
		}
	...
	case "writeText":
		return true, client.Write(sourcePath, []byte(stringValue(request.Args[2])))
	case "mkdir":
	...
	case "copyToLocal":
		total, _ := networkPathSize(client, sourcePath)
		return true, copyNetworkToLocal(client, sourcePath, stringValue(request.Args[2]), &copyProgress{total: total, publish: progress})
	case "copyFromLocal":
		total, _ := localPathSize(stringValue(request.Args[1]))
		return true, copyLocalToNetwork(client, stringValue(request.Args[1]), secondPath, &copyProgress{total: total, publish: progress})
	case "dirSize":
...
func (s *Server) openNetworkFileClient(ctx context.Context, kind string, value any) (networkFileClient, error) {
	...
	sshConnection, err := ssh.Dial("tcp", net.JoinHostPort(host, fmt.Sprint(port)), &ssh.ClientConfig{
		...
		HostKeyCallback: func(hostname string, _ net.Addr, key ssh.PublicKey) error {
			return s.verifySSHHostKey(host, port, key)
		},
...
func networkDirSize(client networkFileClient, root string) (map[string]any, error) {
	...
	walk = func(dir string) error {
		...
		for _, entry := range entries {
			child := path.Join(dir, stringValue(entry["name"]))
