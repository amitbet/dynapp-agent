...
func TestScreenCaptureValidationAndPNG(t *testing.T) {
	...
	if rpcCapability(message{Service: "screen"}) != "screen.capture" || permissionDanger("screen.capture") != "very-high" {
		t.Fatal("screen capture must require an explicit sensitive grant")
	}
}
